...
import { wethContractDataWithMode } from 'src/app/features/instant-trade/services/instant-trade-service/providers/uni-swap-v3-service/controllers/constants/wethContractData';
...
export class EthAndWethSwapController {
  ...
  public isEthAndWethSwap(fromTokenAddress: string, toTokenAddress: string): boolean {
    return (
      (this.web3Public.isNativeAddress(fromTokenAddress) &&
        toTokenAddress.toLowerCase() === this.wethContractData.address.toLowerCase()) ||
      (this.web3Public.isNativeAddress(toTokenAddress) &&
